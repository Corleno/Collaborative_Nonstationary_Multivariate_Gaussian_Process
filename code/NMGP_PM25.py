...
def VTVLCM(data, M, batchsize=0, lr=0.01, itnum=2000, do_inference=True, do_test=False):
    ...
    if do_inference:
        hyperpars = {"length_scales_L0_log": 10, "length_scales_L1_log": 10, "length_scales_tildeell_log": 10}
        initpars = {"mu_v": 1*np.ones(M)}
        if do_test:
            NMGP_model, loss_list, rmse_test_list, time_list = inference(X_train_list, Y_train_list, z, batch_size,
                                                                         dim_outputs, lr=lr,
                                                                         itnum=itnum, hyperpars=hyperpars, verbose=True,
                                                                         show_ELBO=False,
                                                                         X_test_list=X_test_list,
                                                                         Y_test_list=Y_test_list, **initpars)
        else:
            NMGP_model, loss_list, time_list = inference(X_train_list, Y_train_list, z, batch_size, dim_outputs,
                                                                     X_test_list=X_test_list, Y_test_list=Y_test_list, lr=lr,
                                                                     itnum=itnum, hyperpars=hyperpars, verbose=True, **initpars)
        fig = plt.figure()
...
# NMGP_model, loss_list50, rmse_test_list50, time_list50 = VTVLCM(data, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=False)
# # fig = plt.figure()
...
#
# # VTVLCM(data, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=True)
# NMGP_model, loss_list100, rmse_test_list100, time_list100 = VTVLCM(data, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=False)
# # fig = plt.figure()
...
#
# # VTVLCM(data, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=True)
# NMGP_model, loss_list200, rmse_test_list200, time_list200 = VTVLCM(data, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=False)
# # fig = plt.figure()
...
#
fig = plt.figure()
# plt.plot(time_1000[time_1000 < 210], rmse_test_1000[time_1000 < 210], label="BS=1000")
plt.plot(time_1000, rmse_test_1000, label="BS=1000")
...
plt.xticks(ticks=np.linspace(0, 1600, num=6), fontsize=22)
plt.yticks(ticks=np.linspace(0.7, 1.1, num=5), fontsize=22)
plt.legend(fontsize=16)
plt.tight_layout()
plt.savefig("train_trace_BS_PM25.png")
plt.show()
