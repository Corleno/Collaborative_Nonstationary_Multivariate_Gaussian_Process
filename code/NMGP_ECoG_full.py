...
def load_ECoG(data_file, n_channel=None, channel_indexes=None, do_plot_raw_data=True, time_start=14., time_stop=16., rate=400.):
    ...
    return X_train_list, Y_train_list, X_train_vec, \
           Y_train_vec, train_index, \
           t_max, rate, times_test, data, pre_name, time_interval_name
...
def CNMGP(directory, X_train_list, Y_train_list, M, hyperpars={}, initpars={}, batchsize=0, lr=0.01, itnum=2000, do_inference=True, do_earlystop=False, do_stop_criterion=False,
    ...
    if do_inference:
        NMGP_model, loss_list, time_list = inference(X_train_list, Y_train_list, z, batch_size, dim_outputs,
            lr=lr, itnum=itnum, hyperpars=hyperpars, do_earlystop=do_earlystop, do_stop_criterion=do_stop_criterion,
...
if __name__ == "__main__":
    ...
    time_stop = 16.
    rate=400.
    X_train_list, Y_train_list, X_train_vec, Y_train_vec, train_index, \
    t_max, rate, times_test, data, pre_name, time_interval_name = load_ECoG(data_file, time_start=time_start,
    ...
    if do_NMGP50:
        ...
        #       PATH=directory + "/model.pt", continuous_training=True, verbose=True, show_ELBO=False, save_model=True)
    ...
    if do_NMGP100:
        ...
        # CNMGP(directory, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=True)
        NMGP_model, loss_list100, rmse_test_list100, time_list100 = CNMGP(directory, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=False)
        fig = plt.figure()
        ...
        plt.close(fig)
        fig = plt.figure()
        plt.plot(np.array(time_list100), np.array(rmse_test_list100))
        plt.title('rmse_test_trace')
        plt.show()
        plt.close(fig)

        # print("training time per epoch: {}s".format(time_list100[-1]/itnum/math.ceil(X_train_vec.shape[0]/batchsize)))
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig("../res/ECoG/prediction_dim8/prediction_M100.png")
        plt.show()
        plt.close(fig)
        import pdb; pdb.set_trace()
    ...
    if do_NMGP200:
        ...
        # CNMGP(directory, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=True)
        NMGP_model, loss_list200, rmse_test_list200, time_list200 = CNMGP(directory, M, batchsize=batchsize, lr=lr, itnum=itnum, do_inference=False)
        fig = plt.figure()
        ...
        plt.close(fig)
        fig = plt.figure()
        plt.plot(np.array(time_list200), np.array(rmse_test_list200))
        plt.title('rmse_test_trace')
        plt.show()
        plt.close(fig)

        # print("training time per epoch: {}s".format(time_list200[-1]/itnum/math.ceil(X_train_vec.shape[0]/batchsize)))
    ...
    for m in range(n_channels):
        ...
        # plt.xticks(ticks=np.linspace(0, 2000, num=6), labels=np.linspace(0,5,num=6) ,fontsize=18)
        plt.xticks(ticks=np.linspace(0, 800, num=6), labels=np.round(np.linspace(0, 2, num=6), 2), fontsize=18)
        plt.yticks(fontsize=18)
        plt.ylim(-2, 8)
    ...
    if do_distance_analysis:
        ...
        for index, dist in enumerate(dists):
            ...
            # axes[index].set_xticklabels(labels=np.round(np.linspace(0, 5, num=6),2), fontsize=16)
            axes[index].set_yticks(ticks=np.linspace(0, 0.6, num=4))
            axes[index].set_yticklabels(labels=np.round(np.linspace(0, 0.6, num=4),2), fontsize=16)
            axes[index].set_xlabel("Time (second)", fontsize=18)
            axes[index].set_ylabel("Correlation coefficient", fontsize=18)
